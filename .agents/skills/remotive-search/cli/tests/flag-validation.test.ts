...
describe("mapJobs", () => {
  test("maps the portal fixture into the shared Job shape", () => {
    ...
    for (const j of jobs) {
      ...
      expect(j.source).toBe("remotive-search")
  ...
  test("tolerates malformed entries without throwing", () => {
    ...
    test("reads the nested jobs[] array and preserves the listing URL", () => {
    const jobs = mapJobs(FIXTURE)
    expect(jobs[0].url).toContain("remotive.com")
    expect(jobs[0].company).toBe("Globex")
  })
})

const FIXTURE: any = { "jobs": [
  { "id": 2091070, "title": "Senior Full Stack Engineer", "company_name": "Globex",
    "candidate_required_location": "Latin America", "url": "https://remotive.com/remote-jobs/software-dev/senior-full-stack-2091070",
    "publication_date": "2026-07-20T22:25:52+00:00", "salary": "$80k - $110k", "tags": ["react", "node"],
    "job_type": "full_time", "category": "Software Development", "description": "<p>React + Node role.</p>" }
] }

const EMPTY_FIXTURE: any = { "jobs": [ { "id": 3 } ] }
