...
version: 1.0.0
description: >
  Search live remote job listings from We Work Remotely (Global remote board (RSS category feeds)). Best for Felipe's
  remote-international / Americas-timezone software roles. Trigger phrases:
  find jobs, job search, search jobs, remote jobs, We Work Remotely jobs, new positions,
  /scrape.
context: fork
...
Search live job listings from **We Work Remotely** via its public API — no authentication,
...
Uses We Work Remotely's public API. **Keep request volume low** (a couple of calls a day is
plenty — the data barely changes faster than that) and always **apply through the
job's own listing URL** so We Work Remotely keeps its attribution and referral traffic. Do
...
(feed: `programming` [default], `full-stack`, `front-end`, `back-end`, `devops`, `design`),
`--jobage`, `--limit/-n`, `--format`.
...
## Usage examples
```bash
bun run src/cli.ts search -q "react" -c full-stack --format table
```
...
source`. `table` and `plain` are for human scanning. Errors go to
**stderr** as `{ "error": "...", "code": "..." }` with exit code 1.

## Notes
- Source is RSS category feeds; `--category` selects the feed, `--query` filters within it.
- Titles are `Company: Role`; the CLI splits them.
- No salary data in the feed.
