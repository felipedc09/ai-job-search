...
version: 1.0.0
description: >
  Search live remote job listings from RemoteOK (Global remote-first tech board). Best for Felipe's
  remote-international / Americas-timezone software roles. Trigger phrases:
  find jobs, job search, search jobs, remote jobs, RemoteOK jobs, new positions,
  /scrape.
context: fork
...
Search live job listings from **RemoteOK** via its public API — no authentication,
...
Uses RemoteOK's public API. **Keep request volume low** (a couple of calls a day is
plenty — the data barely changes faster than that) and always **apply through the
job's own listing URL** so RemoteOK keeps its attribution and referral traffic. Do
...
```
Key flags: `--query/-q`, `--jobage <days>`, `--remote remote|hybrid|onsite`,
`--limit/-n` (default 20), `--page`, `--format json|table|plain`.
...
## Usage examples
```bash
bun run src/cli.ts search -q "react senior" --jobage 14 --format table
```
...
## Notes
- The API's first array element is a legal/ToS notice; the CLI skips it.
- All listings are remote; `--remote onsite` returns nothing.
- Salaries are annual USD when present (`0`/absent → omitted).
